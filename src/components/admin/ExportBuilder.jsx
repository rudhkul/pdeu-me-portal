...
function SortableColumn({ id, label, onRemove }) {
  ...
  return (
    <div ref={setNodeRef}
      ...
      className={`flex items-center gap-2 bg-white dark:bg-gray-700 border rounded-lg px-3 py-2 text-sm shadow-sm select-none
...
export default function ExportBuilder() {
  ...
  async function loadPreview() {
    ...
    try {
      ...
      setRows(data); setPreviewReady(true)
  ...
  async function doExport() {
    ...
    await exportToExcel(rows, chosen, tab.name,
  ...
  async function exportAllTabs() {
    ...
    } catch (e) { toast.error('Export failed: ' + e.message, { id: 'exportall' }) }
    finally { setExportingAll(false) }
  }

  const panelCard = 'card space-y-4'
  const sectionTitle = 'font-semibold text-gray-800 dark:text-gray-100'
  const addBtn = 'w-full text-left px-3 py-1.5 text-sm text-gray-600 dark:text-gray-300 hover:bg-pdeu-light dark:hover:bg-gray-700 hover:text-pdeu-blue rounded-lg'
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="card mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <p className={sectionTitle}>Export All Tabs at Once</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
            Downloads all {TABS.length} tabs as one Excel file — one sheet per tab. Filters below also apply.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          <div className={panelCard}>
            <h2 className={sectionTitle}>1. Select Tab</h2>
          ...
          <div className={panelCard}>
            <h2 className={sectionTitle}>
              2. Filters <span className="text-xs font-normal text-gray-400">(applies to Export All too)</span>
            </h2>
            <div>
          ...
          <div className={panelCard}>
            <div className="flex items-center justify-between">
              <h2 className={sectionTitle}>3. Add Columns</h2>
        ...
        <div className="xl:col-span-2 space-y-5">
          <div className="card">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className={sectionTitle}>4. Order Columns</h2>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Drag to reorder · {chosen.length} selected</p>
              </div>
              <button onClick={() => setChosen([])} className="text-xs text-red-400 hover:underline">Clear all</button>
            </div>

            {chosen.length === 0
              ? <div className="text-center py-8 text-gray-300 dark:text-gray-600 border-2 border-dashed dark:border-gray-700 rounded-xl">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-72 overflow-y-auto pr-1">
                      {chosen.map((col, i) => (
                        <div key={col.key} className="flex items-center gap-2">
                          <span className="text-xs text-gray-300 dark:text-gray-600 w-5 text-right">{i + 1}</span>
                          <SortableColumn id={col.key} label={col.label}
                            onRemove={k => setChosen(p => p.filter(c => c.key !== k))} />
                        </div>
                      ))}
                    </div>
                  </SortableContext>
          ...
          {previewReady && rows.length > 0 && (
            <div className="card p-0 overflow-hidden">
              <div className="px-4 py-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                <p className="font-semibold text-gray-800 dark:text-gray-100 text-sm">Preview — first 10 rows</p>
                <p className="text-xs text-gray-400 dark:text-gray-500">{rows.length} total rows</p>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead className="bg-pdeu-light dark:bg-gray-700">
                    <tr>
                      {chosen.map(c => (
                        <th key={c.key} className="text-left px-3 py-2 text-pdeu-blue dark:text-blue-400 font-semibold uppercase tracking-wide whitespace-nowrap">
                          {c.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {rows.slice(0, 10).map((row, i) => (
                      <tr key={i} className={`border-b border-gray-50 dark:border-gray-700 ${i % 2 === 1 ? 'bg-gray-50/50 dark:bg-gray-800/60' : ''}`}>
                        {chosen.map(c => (
                          <td key={c.key} className="px-3 py-2 text-gray-600 dark:text-gray-300 max-w-[160px] truncate">
                ...
                {rows.length > 10 && (
                  <p className="text-center text-xs text-gray-400 dark:text-gray-500 py-2">…and {rows.length - 10} more rows</p>
                )}
              </div>
            </div>
          )}
